...
function ItemDAO(database) {
        ...
        */
		...
		this.db.collection("item").aggregate([
			...
			for(var i=0;i<result.length;i++){
				category = {_id: result[i]._id, num: result[i].num};
				categories.push(category);
				total += result[i].num;
			}
			category = {_id: "All", num: total};
			categories.push(category);			
			callback(categories);
		});
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
		...
		// this.db.collection("item").find({_id: itemId}, function(err, doc){
			// if(doc.reviews != undefined) {
				// for(var i=0; i< doc.reviews.length; i++) {
					// doc.reviews[doc.reviews.length+1] = reviewDoc;
				// }
			// } else {
				// doc.reviews = reviewDoc; 
			// }
			
			// this.db.collection("item").update({_id: itemId}, {$addToset : {"doc.reviews": doc.reviews}}, function(err,doc){
				// callback(doc);
			// });
		// });
